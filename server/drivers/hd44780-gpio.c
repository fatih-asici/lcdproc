...
void gpio_HD44780_senddata(PrivateData *p, unsigned char displayID, unsigned char flags,
			   unsigned char ch);
...
export_gpio(int num)
{
	...
	/* If EBUSY is returned, it means this GPIO is already exported.
...
setup_gpio_pin(Driver *drvthis, gpio_pin *pin)
{
	...
	if (export_gpio(pin->number) == -1) {
		report(RPT_ERR, "setup_gpio_pin: unable to export GPIO %d", pin->number);
		return -1;
	}
	...
	/* Set last_value to '-1' in order to force setting of value
	 * at the first setup_gpio_pin() call for this pin. */
	pin->last_value = -1;
...
open_failed:
	report(RPT_ERR, "setup_gpio_pin: unable to open %s for writing", buf);
...
/**
 * Close the file descriptor used by the pin and unexport the GPIO node in sysfs.
 *
 * \param pin   Pointer to gpio_pin structure.
 */
static void
...
hd_init_gpio(Driver *drvthis)
{
	...
	if (setup_gpio_pin(drvthis, &pins->en) == -1 ||
	    setup_gpio_pin(drvthis, &pins->rs) == -1 ||
	    setup_gpio_pin(drvthis, &pins->d7) == -1 ||
	    setup_gpio_pin(drvthis, &pins->d6) == -1 ||
	    setup_gpio_pin(drvthis, &pins->d5) == -1 ||
	    setup_gpio_pin(drvthis, &pins->d4) == -1) {
	...
	if (p->have_backlight) {
		if (setup_gpio_pin(drvthis, &pins->bl) == -1) {
			report(RPT_WARNING,
 ...
 */
void
gpio_HD44780_senddata(PrivateData *p, unsigned char displayID, unsigned char flags,
		      unsigned char ch)
{
	gpio_pins *pins = (gpio_pins *) p->connection_data;
